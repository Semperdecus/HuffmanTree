...
public final class Huffman {
    ...
    public Huffman(String word) throws IOException {
        //Stap 1: Frequentie van de tekens tellen
        inputString = word;

        //Samenzetten tekens in lijst
        ...
        HuffmanTree tree = createTree(freqList);

        //Stap 4: Aflezen van de codes
        codesMap = new HashMap<>();

        //Elke nuttige letter in codesmap zetten
        getCodes(tree, "");
        ...
        //elke 0 of 1 is een byte
        //System.out.println("Amount of bytes: " + codedMessage.length());

        //Stap 6: Opslaan van het bericht
        File encodedfile = serializeMessage(codedMessage);
        System.out.println("Size of encoded file is: " + encodedfile.length() + " bytes down from about " + word.length() + " bytes (UTF-8)");
        //Stap 7: Decoderen
    ...
    public String decodeMessage(String inputMessage, HuffmanTree tree) {
        // Create a string which will hold the message that needs to be decoded further
        String message = inputMessage;
        String decodedmessage = "";

        // Loop until message is decoded
        while (message.length() > 0) {
            int characterCount = 0;
            CharFrequency decodedCharacter = null;
            HuffmanTree node = tree;

            while (decodedCharacter == null) {
                if (!node.getIsCharacterNode()) {
                    ...
                    // Character found and saved
                    decodedCharacter = node.getCharacter();
                }
            }

            decodedmessage += decodedCharacter.getCharacter();
    ...
    public void getCodes(HuffmanTree node, String previousSteps) {
        if (!node.getIsCharacterNode()) {
    ...
    //serialize bitset naar object
    private static File serializeMessage(String message) throws IOException {
        File result;
        final BitSet bitSet = getBitSet(message);

        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(System.getProperty("user.home") + "/Desktop/encodedmessage.txt" ))) {
            oos.writeObject(bitSet);
        }
        result = new File(System.getProperty("user.home") + "/Desktop/encodedmessage.txt" );
        return result;
    }

    //binary text omzetten naar bitset
    ...
    public static void main(String[] args) throws IOException {
        Huffman huffcode = new Huffman(WORD);
    }

    public HuffmanTree createTree(List<CharFrequency> freqList) {
        List<HuffmanTree> nodes = new ArrayList<>();

        for (CharFrequency chr : freqList) {
            nodes.add(new HuffmanTree(chr));
        }

        //loop door elke node van de tree
        while (nodes.size() != 1) {
            // sorteer de kleinste nodes vanonder
            nodes.sort(null);

            // laagste nodes
            HuffmanTree leftChild = nodes.get(0);
            HuffmanTree rightChild = nodes.get(1);

            // verwijder oude nodes van tree
            nodes.remove(leftChild);
            nodes.remove(rightChild);

            // nieuwe node aan tree
            nodes.add(new HuffmanTree(leftChild, rightChild));
        }
        return nodes.get(0);
    }
}
